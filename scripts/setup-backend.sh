...
# バックエンド(Node.js + TypeScript)プロジェクトにESLint/Prettier設定を適用するスクリプト
# Usage: ./setup-backend.sh [PROJECT_PATH]

set -e

PROJECT_PATH=${1:-"."}
CONFIG_DIR="$(dirname "$0")/../backend-nodejs"

echo "🚀 バックエンド設定をセットアップ中..."
